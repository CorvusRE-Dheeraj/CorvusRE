// Deploy via CLI: `supabase functions deploy cancel-bpp-subscription`.
// Requires STRIPE_SECRET_KEY. Mirrors cancel-property-subscription/index.ts
// exactly, swapping properties/propertyId for bpp_accounts/bppAccountId.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!account?.stripe_subscription_id) {
      return new Response(
        JSON.stringify({ error: "This BPP account has no active subscription to cancel." }),
