...
export const earliestBookableDate = (nowMs = Date.now()) =>
  addDays(centralToday(nowMs), MIN_DAYS_AHEAD);
export const latestBookableDate = (nowMs = Date.now()) =>
  addDays(centralToday(nowMs), MAX_DAYS_AHEAD);
...
export function slotProblem(
  ...
  if (!SLOT_HOURS.some((h) => slotKey(h) === slot))
    return "That time isn't one of our appointment slots.";
...
export function openSlotsForDate(
  ...
  return SLOT_HOURS.map(slotKey).filter(
    (s) => slotProblem(date, s, booked, blocks, nowMs) === null,
  );
}
